...
import vinhnd.carshop.cart.CartDTO;
import vinhnd.carshop.user.UserDTO;
        ...
        try {
            ...
            if (con != null) {
                for (CarDTO car : cart.getCart().values()) {
                    String sql = "INSERT INTO OrderDetails(orderID, carID, "
                            + " quantity, salePrice, totalPrice) "
                            + " VALUES(?,?,?,?,?) ";
                    stm = con.prepareStatement(sql);
                    stm.setInt(1, orderID);
                    stm.setString(2, car.getCarID());
                    stm.setInt(3, car.getQuantity());
                    double salePrice = car.getPrice()*car.getQuantity()*
                            (car.getSale()/100);
                    stm.setDouble(4, salePrice);
                    double total = car.getPrice()*car.getQuantity() - salePrice;
