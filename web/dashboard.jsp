        ...
        <header>
            ...
            <div class="searchbar">
                <form action="MainController">
                    <input type="text" name="search" value="${param.search}" placeholder="Search for car"/>
                    <select name="status">
        ...
        <c:if test="${requestScope.CAR_LIST != null}">
            <div class="table">
                <table border="1">
                    ...
                    <tbody>
                        <c:forEach var="car" varStatus="counter" items="${requestScope.CAR_LIST}">
                            <tr>
                        <form action="MainController" method="POST">
                            ...
                            <td>
                                <input type="number" name="quantity" value="${car.getQuantity()}" min="1" required=""/>
                            </td>
                            <td>
                                <input type="number" name="price" value="${car.getPrice()}" min="100" max="1000000000" required=""/>
                            </td>
                            <td>
                                <input type="number" name="sale" value="${car.getSale()}" min="0" max="60" required=""/>
                            </td>
                            <c:forEach var="category" items="${requestScope.CATEGORY}">
                                <c:if test="${car.getCategoryID() eq category.getCategoryID()}">
                            ...
                            <td>
                                <c:if test="${param.status eq true || param.status eq null}">
                                    <c:url var="delete" value="MainController">
                                        ...
                                        <c:param name="carID" value="${car.getCarID()}"/>
